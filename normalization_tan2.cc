...
typedef itk::Image< float, Dimension > Flima;
ImageType::IndexType idx2;
ImageType::IndexType idx3;

std::vector<float> const sigmaV4All {3.0f, 6.0f};
...
ImageType::Pointer combineEenergyBands2Image(std::vector< Flima::Pointer >  energyBandsSource,
                                                     ImageType::Pointer &maskSource){
...
std::vector< Flima::Pointer >  getEnergyBands(ImageType::Pointer  &image, std::vector<float> sigmaV) {
    ...
    typedef itk::CastImageFilter< ImageType,Flima> CastSint2Float;
    auto inputSintFloat = CastSint2Float::New();
    inputSintFloat->SetInput(image);
    inputSintFloat->Update();
    ...
    Flima::Pointer lastLPBand=inputSintFloat->GetOutput();
...
void getmask(ImageType::Pointer const &imageGrey, ImageType::Pointer &mask){

    typedef itk::OtsuThresholdImageFilter <ImageType, ImageType>
            FilterType;
    FilterType::Pointer otsuFilter
            = FilterType::New();
    otsuFilter->SetInput(imageGrey);
    ...
    //ImageType::Pointer maskcopy0=labelShapeKeepNObjectsImageFilter->GetOutput();
    ImageType::Pointer maskcopy0=otsuFilter->GetOutput();

    ImageType::SizeType sizeSource = imageGrey->GetLargestPossibleRegion().GetSize();
        ...
        for(size_t j = 0; j< sizeSource[1]; j++) {
            for(size_t i = 0; i< sizeSource[0]; i++)  {
                ...
                if (maskcopy0->GetPixel(idx3)> 0){
                    mask->SetPixel(idx3,0);
                }else{
                    mask->SetPixel(idx3,255);
                }
            }
    }
...
//    for(size_t k = 0; k< sizeSource[2]; k++) {
        for(size_t j = 0; j< sizeSource[1]; j++) {
            for(size_t i = 0; i< sizeSource[0]; i++)  {
                ...
                idx2[1]=j;
//                idx3[2]=k;
                if (imageSouce->GetPixel(idx2)>300){
...
//    }
    ...
    getmask(imageTarget, maskTarget);
...
//                idx3[2]=k;
                ...
                if (imageTarget->GetPixel(idx2)>300){
                    maskTarget->SetPixel(idx2,1);
                }
            }
        }
//    }
    ...
    writer->SetFileName("maskSource.mhd");
    writer->SetInput(maskSource);
    ...
    writer->SetFileName("maskTarget.mhd");
    writer->SetInput(maskTarget);
